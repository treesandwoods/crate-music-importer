...
- **Playlists Browse & Import** keeps **Import a New Playlist** first and lists prior imports below it with center-cropped square artwork and no rounded corners. A new public Spotify link follows the same preview, confirmation, matching, download, and exact-ID validation workflow. Selecting a saved playlist previews an occurrence-aware update: Spotify-only reorders are ignored, additions are placed relative to surviving imported entries, and manual Music entries retain their order. Every removal is labeled **unlink** or **delete permanently** before confirmation.
- **Review Activity & Problems** shows the durable FIFO job queue, per-track progress, completed history, review choices, resumable failures, retry actions, and cancellation for incomplete work. Jobs continue after Raycast closes. Completion, failure, and approval-needed events use the separate **Import Notifications** background command to display a separate HUD when Raycast is closed and a normal toast when its window is visible. Background delivery hands off to a foreground no-view command for the toast, because Raycast forbids the Toast API in background launches. An open Raycast window is not closed for a HUD. Allow that command if Raycast requests launch confirmation. Events remain pending until delivery succeeds, including a new completion after a reviewed job is retried.
...
Previews use the persistent cache, but final writes validate exact persistent IDs again. Importer-owned playlist recordings may be upgraded in place for a real album only when ownership is proven. User-owned tracks, playlist memberships, and surviving playlist order are preserved. Crate Music Importer never changes Finder or iPod whole-library sync settings; continue using your normal sync afterward.
