...
Music.app is authoritative. Previews use a disposable cache that is rebuilt from Music.app when missing or invalid, while final writes validate exact persistent IDs again. A Music track is reusable regardless of how it entered the library. Crate changes or removes a physical file only when the registered path and hashes prove that file is Crate-managed. Full playlist updates change only the selected playlist membership, and Crate Music Importer never changes Finder or iPod whole-library sync settings.
