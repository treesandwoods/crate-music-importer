...
- **Unify Music Library**: read a saved canonical path preview, deliberately scan, then explicitly approve organization, resume, or rollback.
- **Review Crate Working Files**: inspect active, recoverable, completed, and unknown staging files; eligible sidecars can be explicitly moved to Trash.
...
Finished imports use `Music/Album Artist/Album/01 — Song Title.mp3`, or `Music/Compilations/Album/` for compilations. Multi-disc albums use `2-03 — Song Title.mp3`. Missing values use Unknown Artist and Unknown Album. Existing `tracks/` files remain until explicitly organized. `.state/` contains the catalog, derived cache, jobs, logs, and recovery journals; `.staging/` contains unfinished or recoverable material. Never publish these or downloaded media. Changing the configured path selects another library; it does **not** migrate or merge existing content. Preserve the complete folder when moving it deliberately.
...
The health audit deliberately reads the full Music library (persistent IDs, locations, comments, duration, tags), the configured manifest, and the existing Music cache. It checks registered managed files even when absent from Music, and reads local files for existence, readability, nonzero size, SHA-256 and FFprobe duration. Every local file receives full FFmpeg decoding, audio fingerprinting, metadata completeness, and artwork checks, regardless of provenance. **Deep Check All Local Music** recomputes these checks for all local files. Cloud-only entries are informational. Exact byte duplicates and possible artist/title/version/duration matches are separate findings. Source-specific metadata comparisons remain limited to trustworthy source records and honor playlist/album metadata profiles, and use the existing duration tolerances. Artwork bytes are decoded locally; Music's displayed artwork and remote artwork URLs are not fetched.

The audit distinguishes stale saved paths from missing songs: an absent historical path is informational only when the same unambiguous Music persistent ID points to audio that passes the current file checks. These references are grouped into one finding; nonexistent paths do not count as Spotify duplicate files. A shared storage folder alone does not establish importer ownership. Older records without a saved metadata profile or artwork-source URL do not imply incorrect album tags or missing artwork; actual embedded artwork is still checked. Changed file fingerprints remain review warnings because tags and artwork edits also change whole-file SHA-256. The audit never silently reconciles the manifest or cache.

Each finding includes severity, ownership, track metadata, persistent and recording IDs, paths, evidence and a suggested manual review step. Ordinary user-owned tracks receive catalog entries on rebuild; optional source recording records are not required. Findings offer diagnostic copying and Reveal in Finder, with no repair, delete or rewrite actions. The latest report is written atomically to `<managed directory>/.state/health-last-result.json`, including failed scans. This diagnostic contains local paths and track metadata; treat it as private. The scan does not write Music, media, playlists, manifests, the Music cache, or Finder/iPod settings. Matching previews read a rebuildable view derived from the catalog. Catalog refreshes do not move media. Health and dependency checks hold shared dependency locks; updates require the exclusive lock. Lock conflicts are reported, never bypassed.
...
## Unifying an existing library

Music remains responsible for visible metadata, artwork, ratings, favorites, play history, and ordered user playlists. The catalog stores exact Music IDs, current locations, integrity baselines, and optional source links. Provenance does not control organization or health coverage and does not grant permission to rewrite an adopted file.

Open **Unify Music Library**, confirm that both options in **Music Settings > Files** are unchecked, and run a fresh preview. That observation is bound to preference-file fingerprints: any change invalidates it and requires another observation. The saved preview includes every Music entry, finished MP3 inventory, proposed readable paths, collisions, missing or unsupported files, duplicate candidates, recovery material, and required copy/backup space. The first preview only writes private reports in `.state/`; it never moves or retags audio.

Review the exact preview before **Apply Verified Organization Plan**. A stale preview or unresolved prerequisite stops execution. Organization copies files, checks complete bytes and decoded audio, relinks existing persistent IDs, checks preservation, commits the catalog, and only then moves originals to their volume's Trash. The journal supports resume and rollback. The recovery package backup remains until **Accept Result and Review Backup** is explicitly chosen; Trash is never emptied automatically. After accepting and trashing the backup, restore that backup before using automated rollback.

Metadata edits and in-place album upgrades retain existing paths. Run organization again to review resulting path changes. New Music files outside the configured root appear as awaiting adoption on the next preview. New Crate imports use the same canonical tree. Generated M3U playlist comments, order, and repeated entries are preserved while paths are rewritten.

The schema is additive within the existing manifest, with a pre-catalog backup when a complete catalog is first committed. Source records keep compatibility fields for import workflows and link to permanent catalog IDs. Cache files can be deleted and reconstructed from the catalog. Rebuild verifies local files without replacing an existing integrity baseline merely because current audio differs.

Working-file review never treats finished MP3s as disposable staging. Unknown material and recovery audio remain protected. Only verified, unreferenced downloader description sidecars are eligible for the explicit Trash action.
