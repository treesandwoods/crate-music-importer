...
- **Review Activity & Problems** shows the durable FIFO job queue, per-track progress, completed history, review choices, resumable failures, retry actions, and cancellation for incomplete work. Jobs continue after Raycast closes. Completion, failure, and approval-needed events use the separate **Import Notifications** background command to display compact Raycast toasts (or a HUD when Raycast is closed). Allow that command if Raycast requests launch confirmation. Events remain pending until delivery succeeds, including a new completion after a reviewed job is retried.
